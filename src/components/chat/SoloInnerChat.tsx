...
export function SoloInnerChat({ myPayload, zhihuItems }: SoloInnerChatProps) {
  ...
  const generateMessage = useCallback(
    async (
      speakingRole: "superego" | "id",
      lastPartnerMessage: string,
    ): Promise<string> => {
      const isSuperego = speakingRole === "superego";
      const currentSessionId = isSuperego
        ? superegoSessionIdRef.current
        : idSessionIdRef.current;
...
4. 绝对不要加任何角色名或动作描写，只输出说话内容。`;
  ...
  const typewriterDisplay = useCallback(
    async (text: string, role: "superego" | "id"): Promise<void> => {
      setIsTyping(true);
      setTypingRole(role);
      setDisplayedText("");
  ...
  const startConversation = useCallback(async () => {
    ...
    for (let round = 0; round < TOTAL_ROUNDS; round++) {
      ...
      const speakingRole: "superego" | "id" =
        round % 2 === 0 ? "id" : "superego";
    ...
    <div className="w-full flex-1 flex flex-col animate-[fadeIn_0.5s_ease-out] mt-6 mb-12">
      ...
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-brand-slate-800">
        <div className="flex items-center gap-3">
          <div className="flex flex-col gap-1">
            <span className="text-[10px] text-brand-slate-500 font-bold uppercase tracking-widest">
              [ 单人精神推演沙盘 ]
            </span>
        ...
        <div className="flex flex-col items-end">
          ...
          <div className="w-24 h-1 bg-brand-slate-800 rounded-full overflow-hidden">
            <div
              className="h-full bg-brand-indigo-500 transition-all duration-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
      ...
      <div className="flex-1 space-y-5 pb-4">
        {messages.map((msg, i) => (
          <div
            key={i}
            className={`flex ${msg.role === "id" ? "justify-start" : "justify-end"}`}
          >
            <div
            ...
            >
              <span
                className={`text-[9px] font-bold tracking-wider block mb-2 opacity-70 uppercase ${msg.role === "id" ? "text-left text-brand-rose-400" : "text-right text-sky-400"}`}
              >
                {msg.role === "id"
                  ? "[ 🩸 原始驱动池 - 本我 ]"
                  : "[ 🛡️ 深度防备中枢 - 超我 ]"}
        ...
        {isTyping && (
          <div
            className={`flex ${typingRole === "id" ? "justify-start" : "justify-end"}`}
          >
            <div
            ...
            >
              <span
                className={`text-[9px] font-bold tracking-wider block mb-1 opacity-50 uppercase ${typingRole === "id" ? "text-left" : "text-right"}`}
              >
                {typingRole === "id"
                  ? "[ 本能生成中... ]"
                  : "[ 理智计算中... ]"}
